...
class Car:
    ...
    def __init__(self):
        ...
        self.speed = random.randint(1,3) / 20
        self.maxspeed = random.randint(15,20) / 10
        self.accel = 0
        self.color = RED
        ...
        self.dt = random.randint(4,8) / 10000
    ...
    def accelerate(self, location):
        self.ACCEL = self.accel * self.accel * self.dt
...
def traffic_signal(wait):       #Decreases the amount and general possibility of an overlay between the pedestrian and the car
    ...
    if(int(current_time) % wait >=0 and int(current_time)%wait<=1):
        pygame.draw.rect(screen, CAUTION, (660,30,10,150))
        pygame.draw.rect(screen,CAUTION, (80,430,150,10))
...
#                    for person_sweep in range(0, 4):    #ANI - CHANGED
...
#                            person_bool = False
            ...
            #person coming from left leg
                ...
                for x in range(10,20):
                    for person_sweep in range(-4, 0):      #ANI-CHANGED
