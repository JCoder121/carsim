...
#Update: Timer, denoted by total_seconds, is created by subtracting the time that it takes for the 30th car to get past the end because this signifies that the parking lot is full. 
import pygame
import random
import math  
import os.path
...
def main():
    ...
    while not done:
        ...
        if car_count == 30 and start_seconds_bool:
            start_seconds = pygame.time.get_ticks() / 1000
            print(start_seconds)
            start_seconds_bool = False
    ...
    if hundred_cars_bool:
        body_print = "\n\nMinutes needed for 100 cars to pass: " + str(float(total_seconds/60))
        print(body_print)
